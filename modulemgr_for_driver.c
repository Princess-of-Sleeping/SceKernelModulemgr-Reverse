...
int SceModulemgrForDriver_0975B104(void *a1){

	int res;
	SceUID modid;
	void *pRes;

	modid = func_0x81007c10(0x10005, a1);
	if(modid < 0)
		return modid;
...
int SceModulemgrForDriver_1D9E0F7E(void *a1, SceKernelModuleInfo *info){
	...
	return func_0x81007790(0x10005, modid, (SceKernelModuleInfo_fix_t *)info);
}

int SceModulemgrForDriver_861638AD(void *a1){
...
int SceModulemgrForDriver_02D3D0C1(SceUID pid, SceUID modid, SceSize args, void *argp, int flags, SceKernelULMOption *option, int *status){
